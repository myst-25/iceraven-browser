...
def update_download_links(file_path, assets):
    ...
    for asset in assets:
        ...
        if 'arm64' in name or 'aarch64' in name:
            asset_map['arm64'] = url
        elif re.search(r'armeabi[-_]v7a', name):  # Strict match for armeabi-v7a
            asset_map['arm-v7a'] = url
        elif 'x86_64' in name or 'x64' in name:
            asset_map['x86_64'] = url
        elif re.search(r'\bx86\b|ia32', name):  # Match exact x86 or ia32
            asset_map['x86'] = url

    # Replace old links with updated URLs
    for arch, url in asset_map.items():
        pattern = rf'https://github\.com/[^"\')\s]+{re.escape(arch)}[^"\')\s]*' 
